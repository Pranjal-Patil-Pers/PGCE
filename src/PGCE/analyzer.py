...
class CFEAnalyzer:
    ...
    def __init__(self, model, feature_names: List[str], min_max_dict: dict | None = None, window_pattern: str = r"\[(.*?)\]"):
        self.model = model
    ...
    def get_feature_importance(self, model=None, feature_names=None) -> pd.DataFrame:
        ...
        sorted_normalized_weights_dict = dict(sorted(normalized_weights_dict.items(), key=lambda item: item[1], reverse=True))
        return pd.DataFrame(list(sorted_normalized_weights_dict.items()), columns=["Feature", "Normalized Importance"])
    ...
    def analyze_counterfactuals(self, df: pd.DataFrame, min_max_dict: dict, query_instance, window_pattern=r"\[(.*?)\]"):
        ...
        features_for_window = [feat for feat in features_sorted_by_change if isinstance(feat, str) and feat != "Event_Y_N"]
        windows = []
        ...
        return {"violations": violations, "changed_features_ordered": changed_features_sorted, "changed_windows_ordered": sorted_windows}
