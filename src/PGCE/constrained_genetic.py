...
class ConstrainedDiceGenetic(DiceGenetic):
    def __init__(self, data_interface, model_interface, constraints: Optional[Sequence[ConstraintLike]] = None, l0_penalty_weight: float = 0.0):
    ...
    def _feature_to_index(self) -> Dict[str, int]:
        if self._feature_to_index_cache is None:
            self._feature_to_index_cache = {name: idx for idx, name in enumerate(self.data_interface.feature_names)}
    ...
    def compute_constraint_penalty(self, candidates: np.ndarray, desired_class) -> np.ndarray:
        ...
        for constraint in self.constraints:
            penalties += evaluate_constraint(constraint=constraint, candidates=candidates, feature_to_index=feature_to_index, query_instance=query_instance, desired_class=desired_class)
        return penalties
    ...
    def compute_loss(self, cfs: np.ndarray, desired_range, desired_class):
        self.yloss = self.compute_yloss(cfs, desired_range, desired_class)
        self.proximity_loss = self.compute_proximity_loss(cfs, self.query_instance_normalized) if self.proximity_weight > 0 else np.zeros(cfs.shape[0], dtype=float)
        self.sparsity_loss = self.compute_sparsity_loss(cfs) if self.sparsity_weight > 0 else np.zeros(cfs.shape[0], dtype=float)
        self.diversity_loss = self.compute_diversity_loss(cfs) if self.diversity_weight > 0 else np.zeros(cfs.shape[0], dtype=float)
        self.constraint_penalty = self.compute_constraint_penalty(cfs, desired_class=desired_class)
        self.l0_penalty = self.compute_l0_penalty(cfs) * self.l0_penalty_weight if self.l0_penalty_weight > 0 else np.zeros(cfs.shape[0], dtype=float)
        total_loss = (
