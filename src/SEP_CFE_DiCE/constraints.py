...
class CounterfactualConstraint(Protocol):
    def penalty(
        ...
        query_instance: Optional[np.ndarray] = None,
        desired_class: DesiredClass = None,
    ) -> np.ndarray:
        """Returns a penalty for each candidate (higher is worse)."""


ConstraintCallable = Callable[
    [np.ndarray, Mapping[str, int], Optional[np.ndarray], DesiredClass],
    ConstraintResult,
]
...
def _as_penalty_vector(raw_penalty: ConstraintResult, n_candidates: int) -> np.ndarray:
    ...
    if penalty.shape[0] != n_candidates:
        raise ValueError(
            f"Constraint returned {penalty.shape[0]} penalties for {n_candidates} candidates."
        )
    return penalty
...
def evaluate_constraint(
    ...
    if hasattr(constraint, "penalty"):
        raw_penalty = constraint.penalty(
        ...
        raise TypeError(
            "Constraint must either expose .penalty(...) or be a callable with signature "
            "(candidates, feature_to_index, query_instance, desired_class)."
        )
...
class FeatureRangeConstraint:
    """Penalizes values outside per-feature min/max ranges."""
...
class OrderedFeaturesConstraint:
    """
    Penalizes order violations in feature groups.

    Example for p3 > p5 > p7:
    ordered_feature_groups=[("p3", "p5", "p7")], increasing=False, strict=True
    """
    ...
    def penalty(
        ...
        for group in self.ordered_feature_groups:
            ...
            for left_idx, right_idx in zip(indices[:-1], indices[1:]):
                ...
                if self.increasing:
                    if self.strict:
                        violations = left_values >= right_values
                    else:
                        violations = left_values > right_values
                else:
                    if self.strict:
...
class FeatureThresholdConstraint:
    """Penalizes values that violate lower/upper threshold bounds."""
    ...
    def penalty(
        ...
        if self.lower_bound is not None:
            if self.lower_inclusive:
                violations |= values < self.lower_bound
            else:
                violations |= values <= self.lower_bound

        if self.upper_bound is not None:
            if self.upper_inclusive:
                violations |= values > self.upper_bound
            else:
                violations |= values >= self.upper_bound
